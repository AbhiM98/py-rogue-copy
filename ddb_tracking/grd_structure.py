...
from ddb_tracking.grd_constants import ProcessFlags, RoguesS3PathStructure, TimeZones
...
class MultiViewMP:
    ...
    def get_root_mp(self):
        ...
        depth = next(
            (
                ...
                if CameraViews.BOTTOM in key_segment.name
            ),
    ...
    def from_root_mp(root_mp: MirrorPath, suffix: str = ".mp4"):
        ...
        return MultiViewMP(
            bottom_mp=root_mp.get_child(f"{CameraViews.BOTTOM}{suffix}"),
            nadir_mp=root_mp.get_child(f"{CameraViews.NADIR}{suffix}"),
            oblique_mp=root_mp.get_child(f"{CameraViews.OBLIQUE}{suffix}"),
        )
...
class GRDRow:
            ...
            for ds_split_number in range(0, self.n_ds_splits):
                ...
                pg = GRDPlantGroup(
                    ...
                    image_directories=MultiViewMP.from_root_mp(
                        ds_split_mps[ds_split_number],
                        suffix=f" {DataFolders.RAW_IMAGES}",
                    ),
    ...
    def get_date_row_number(date: str, row_number: int) -> str:
        """Get the date#row_number format from a date and row_number."""
